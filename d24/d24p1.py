...
class Group:
    ...
    def __init__(self, line, army, index):
        ...
        self.attack_points = int(groups[4])
...
def solve(input):
    ...
    for line in input.strip().split('\n'):
        if line == 'Immune System:':
            ...
            group = Group(line, army, 1 + len([g for g in groups if g.army == army]))
            groups.append(group)
    ...
    while True:
        ...
        for i, group in enumerate(groups):
            enemy_army = INMUNE_ARMY if group.army == INFECTION_ARMY else INFECTION_ARMY
        ...
        print(alive_infection, len(groups), sum(g.units for g in groups if g.army == INFECTION_ARMY), sum(g.units for g in groups if g.army == INMUNE_ARMY))
        if len(groups) == 11:
            print('\n'.join([g.long_repr() for g in groups]))
