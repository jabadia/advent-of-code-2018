...
RE_EVENT = re.compile(r'\[(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2})] (.*)')
...
def solve(input):
    ...
    _, minute_most_asleep = max((sleep_times[guard_most_asleep][minute], minute) for minute in range(0, 60))
    return guard_most_asleep * minute_most_asleep
